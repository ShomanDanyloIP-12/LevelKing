...
class Public_levels_menu:
    def __init__(self, switch, get_public_levels, decide_chosen_strlevel):
        ...
        self.switch = switch
        self.get_public_levels = get_public_levels
        self.decide_chosen_strlevel = decide_chosen_strlevel
        self.buttons = Buttons_plsm()
        self.switch_locker = True
        self.image_tittle = load(
            path.join(script_directory, 'graphics', 'menus', 'public_levels_menu', 'tittle.png')).convert_alpha()
        ...
        self.level_list = ScrollableList(
    ...
    def event_loop(self):
        for event in pygame.event.get():
            ...
            clicked_level = self.level_list.handle_event(event)
            if clicked_level:
                print("User clicked on:", clicked_level)
                self.decide_chosen_strlevel(clicked_level.get('id'))
                self.switch_locker = False
                self.switch({'from': 'public_levels_menu', 'to': 'stranger_level_menu'})
...
class Buttons_plsm:
    ...
    def create_buttons(self):
        ...
        self.public_levels_rect = pygame.Rect(topleft, (width, height))
