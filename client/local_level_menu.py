...
class Local_level_menu:
    def __init__(self, switch,publish_level):
    ...
    def menu_click(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and self.buttons.b_rect.collidepoint(
            ...
            self.switch({'from': 'local_level_menu', 'to': 'server_menu'})
        if event.type == pygame.MOUSEBUTTONDOWN and self.buttons.publish_button_rect.collidepoint(
                mouse_pos()) and self.switch_locker == True:
            self.switch_locker = False
            self.publish_level()
            self.switch({'from': 'local_level_menu', 'to': 'server_menu'})
        if event.type == pygame.MOUSEBUTTONDOWN and self.buttons.redact_button_rect.collidepoint(
                mouse_pos()) and self.switch_locker == True:
            self.switch_locker = False
            self.switch({'from': 'local_level_menu', 'to': 'level_builder'})
...
class Buttons_llm:
    ...
    def create_buttons(self):
        ...
        self.image_publish_button = load(
            path.join(script_directory, 'graphics', 'menus', 'level_buttons',
                      'publish_button.png')).convert_alpha()
        self.image_publish_button = pygame.transform.scale(self.image_publish_button, (sx(180), sy(70)))
        self.publish_button_rect = pygame.Rect(
    ...
    def display(self):
        ...
        self.display_surface.blit(self.image_publish_button, self.publish_button_rect.topleft)
