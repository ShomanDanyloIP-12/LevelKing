...
class Authorisation_s_menu:
    def __init__(self, switch, register_user, log_user, login):
        ...
        self.sign_up = False
        self.image_tittle = load(path.join(script_directory, 'graphics', 'menus', 'authorisation_menu', 'tittle_sign.png')).convert_alpha()
    ...
    def menu_click(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and self.buttons.sign_up_button_rect.collidepoint(
            ...
            if self.register_user(self.input_fields.get_login(), self.input_fields.get_password()):
                ...
                self.input_fields.login_text = 'Invalid 400'
                self.input_fields.login_txt_surface = self.input_fields.font.render(self.input_fields.login_text, True,
                                                                                    '#33323d')
        if event.type == pygame.MOUSEBUTTONDOWN and self.buttons.back_button_rect.collidepoint(
                mouse_pos()) and self.switch_locker == True:
            self.switch_locker = False
...
class Buttons_ams:
    ...
    def create_buttons(self):
        ...
        # button areas
        self.image_back_button = load(
            path.join(script_directory, 'graphics', 'menus', 'authorisation_menu', 'back_button.png')).convert_alpha()
        self.image_back_button = pygame.transform.scale(self.image_back_button, (sx(150), sy(70)))
        self.back_button_rect = pygame.Rect(vector(self.menu_rect.topleft) + (sx(25), sy(265)), (self.menu_rect.width / 2 - sx(50), self.menu_rect.height / 4 - sy(20)))
        self.image_sign_up_button = load(
            path.join(script_directory, 'graphics', 'menus', 'authorisation_menu', 'sign_up_button.png')).convert_alpha()
        self.image_sign_up_button = pygame.transform.scale(self.image_sign_up_button, (sx(150), sy(70)))
        self.sign_up_button_rect = pygame.Rect(vector(self.menu_rect.topleft) + (sx(6) +self.menu_rect.width / 2, sy(265)),
    ...
    def display(self):
        ...
        self.display_surface.blit(self.image_back_button, self.back_button_rect.topleft)
